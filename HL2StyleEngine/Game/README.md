...
- First-pass GLB world prop rendering: rigid bodies can point `MeshPath` at an imported `.glb`, and the renderer fits imported bounds to the existing physics box. GLB node/mesh names are preserved on loaded model parts, and world models can skip named parts at draw time. All current throwable `Crate_*` props use the imported breakable wooden crate model.
...
- Configurable object health for box, rigid-body, and prop entities, with editor-selected broken replacement models and save/load persistence for broken state. Current breakable wooden crates use named GLB fracture parts for staged damage, then collapse remaining pieces into short-lived falling/fading debris instead of swapping to damaged crate models.
